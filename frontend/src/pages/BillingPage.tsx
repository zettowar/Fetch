...
const PERKS = [
  { icon: '↩️', title: 'Unlimited rewinds', body: 'Undo any swipe — no time limit, no daily cap.' },
  { icon: '🐾', title: 'Unlimited daily swipes', body: 'Free users get 50/day. Pack+ removes the limit.' },
  { icon: '🚫', title: 'No ads', body: 'No banners, no rewarded videos, no interruptions.' },
  { icon: '🏆', title: 'Priority winners', body: 'Earlier visibility on the weekly top-dog feed.' },
];
      ...
      <ul className="space-y-3 mb-6">
        {PERKS.map((perk) => (
          <li key={perk.title} className="flex gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/40">
            <span className="text-2xl flex-shrink-0" aria-hidden>{perk.icon}</span>
