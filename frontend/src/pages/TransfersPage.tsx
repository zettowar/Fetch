...
export default function TransfersPage() {
  ...
  const accept = useMutation({
    ...
    onSuccess: () => {
      toast.success('Accepted — the pet is now yours 🎉');
      invalidate();
    },
  ...
  return (
    <div className="p-4 pb-8 max-w-xl mx-auto">
      ...
      <h1 className="text-2xl font-bold mt-2 mb-1 flex items-center gap-2">
        <ArrowLeftRight size={20} aria-hidden className="text-brand-500" /> Pet transfers
      </h1>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">
        When a rescue adopts one of their pets to you, you'll see an invitation
                ...
                {history.map((t) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium truncate">{t.pet_name}</p>
...
function TransferCard({
  ...
  return (
    <div className="p-4 bg-white dark:bg-gray-900 border border-brand-200 dark:border-brand-500/30 rounded-2xl">
      <div className="flex items-center gap-3 mb-3">
        {transfer.pet_photo_url && (
          <img
            src={transfer.pet_photo_url}
            alt={transfer.pet_name ?? 'Pet'}
            className="w-14 h-14 rounded-full object-cover"
          />
        )}
        <div className="flex-1 min-w-0">
          <p className="font-semibold">{transfer.pet_name}</p>
      ...
      <p className="text-sm text-gray-600 dark:text-gray-300 mb-3">
        Accepting takes ownership of <span className="font-medium">{transfer.pet_name}</span> on your account.
      </p>
